...
`系统变量`比较牛逼的一点就是，对于大部分系统变量来说，它们的值可以在服务器程序运行过程中进行动态修改而无需停止并重启服务器。系统变量的`作用范围`分为这两种：
...
- `File Trailer`存储前4个字节代表页的校验和和后4个字节代表页面被最后修改时对应的日志序列位置（LSN），用于数据持久化时判断当前页是否持久化成功
...
**页分裂**：当前页的`Free Space`无了之后，再存入数据，为了保证==下一个数据页中用户记录的主键值必须大于上一个页中用户记录的主键值==，新增加页存储数据。这个过程我们也可以称为`页分裂`。

我们需要给它们做个目录，每个页对应一个目录项，每个目录项包括下边两个部分：

- 页的用户记录中最小的主键值，我们用`key`来表示。
- 页号，我们用`page_no`表示。

以上的目录页又称为==索引==，至于查找的时候首先通过二分查找的方式查找到主键在索引中的值，主键时key，value时存在索引中的值，对应页号，确定页号之后通过页目录二分查找到对应的数据槽遍历查找。
...
聚簇索引只有在搜索主键的时候才能发挥效果，因为聚簇索引中的数据都是按照主键从大到小的数据存储的。当搜索的条件不是主键的时候，由于不能使用聚簇索引，所以我们为这些经常查询的字段建立独立的B+树索引，称为二级索引（空间换时间）
...
当我么读取数据时，首先访问Buffer Pool的缓存，如果缓存中存在寻找的页，直接通过hash找到页数据，否则在磁盘中读取页数据到内存中，由于磁盘太慢，用内存作为缓存很有必要。
