...
当数据量小的时候底层采用ziplist存储，压缩列表不是常规的链表，而是一块连续的内存，通过K+V相间的形式存储数据，这样减少了额外的双向指针占用空间。当数据量大时，采用quickList存储，quickList就是采用双向指针连接ziplist的存储方式，既节省了空间，又保证了数据插入的效率（如果一直采用ziplist会经常扩容，影响插入数据的效率）。
